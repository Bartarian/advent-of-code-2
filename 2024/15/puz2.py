        ...
        if line == "":
            ...
            for letterNum, letter in enumerate(line):
                if letter == "#":
                    ...
                    fullMap[lineNum].append(3)
                else:
                    fullMap[lineNum].append(1)
                    fullMap[lineNum].append(1)
                    robotPosition=(lineNum, letterNum * 2)
    else:
        for element in line:
...
def moveAndPush(row, col, dir):
    ...
    if fullMap[row][col] == 0:
        ...
        if fullMap[destination[0]][destination[1]] == 0:
            ...
            fullMap[row][col] = 1
            return destination
        elif fullMap[destination[0]][destination[1]] in (2, 3):
            # Box, try to move it
            if dir == 1:
                if moveAndPush(destination[0], destination[1] + 1, dir):
                    moveAndPush(destination[0], destination[1], dir)
                    fullMap[destination[0]][destination[1]] = item
                    fullMap[row][col] = 1
                    return destination
            if dir == 3:
                if moveAndPush(destination[0], destination[1] - 1, dir):
                    ...
                    return destination
            if dir == 2:
                mapBackup = deepcopy(fullMap)
                if fullMap[destination[0]][destination[1]] == 2:
                    # [
                    if (
                        moveAndPush(destination[0], destination[1], dir)
                        and moveAndPush(destination[0], destination[1] + 1, dir)
                    ):
                        fullMap[destination[0]][destination[1]] = item
                        fullMap[row][col] = 1
                        return destination
                        ...
                        fullMap = mapBackup
                elif fullMap[destination[0]][destination[1]] == 3:
                    # ]
                    if (
                        moveAndPush(destination[0], destination[1], dir)
                        and moveAndPush(destination[0], destination[1] - 1, dir)
